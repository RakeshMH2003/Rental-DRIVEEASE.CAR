...
/* ============================================================
   DRIVEEASE — app.js  (Fully Fixed & Finalized v3)
   Fixes applied:
   - Admin credentials: rakesh@gmail.com / rakesh@123
   - Vehicle images: upImgs stores raw base64 strings
   - Images display correctly in cards and detail modal
   - Insurance doc viewable by vendor and admin
   - imgCounter replaced with simple array push
   - removeImg works by index
   ...
   - Admin sign-in via email+password (not just demo button)
   - Navbar shows correct active page link
   - Mobile nav closes after navigation
   - Date validation on booking form
   - Filter tags sync correctly on vehicles page
   - safeImgSrc handles both string URLs and legacy {key,data} objects
   - Session restore properly refreshes vendor status
   - Contact form resets all fields after submit
   - Vendor orders tab shows proper booking cards
   - Empty state messages improved
   - sortVendors and sortUsers properly re-render
   - All confirm() dialogs use descriptive messages
   ============================================================ */
...
/* ── ADMIN CREDENTIALS ── */
var ADMIN_EMAIL    = 'rakesh@gmail.com';
var ADMIN_PASSWORD = 'rakesh@123';
var ADMIN_NAME     = 'Rakesh';
...
/* Safe image src — returns the usable src string */
...
/* Get first valid image src from a vehicle's images array */
...
$(function() {
  ...
  $('#vendorSigninModal').on('keydown', function(e) {
    if (e.key === 'Enter') vendorSignIn();
  });
  $('#vendorSignupModal').on('keydown', function(e) {
    if (e.key === 'Enter') vendorSignUp();
  });
  ...
  $(document).on('click', function(e) {
   ...
   $(document).on('click', '#licFrontBox', function() { $('#licFront')[0].click(); });
   $(document).on('click', '#licBackBox',  function() { $('#licBack')[0].click(); });
   $(document).on('click', '#insBox',      function() { $('#carIns')[0].click(); });
   $(document).on('click', '#imgUpBox',    function() { $('#vImgs')[0].click(); });

  /* File change handlers — delegated so they survive resets */
  $(document).on('change', '#licFront', function() { handleLicUpload(this, 'front'); });
  $(document).on('change', '#licBack',  function() { handleLicUpload(this, 'back'); });
  $(document).on('change', '#carIns',   function() { handleInsuranceUpload(this); });
  $(document).on('change', '#vImgs',    function() { handleVehicleImages(this); });

  /* Set hero date min */
  var today = new Date().toISOString().split('T')[0];
  $('#heroPickup, #heroReturn, #bkStart, #bkEnd').attr('min', today);
});
...
function signUp() {
  ...
  // Block admin email from being used to create user account
  if (email === ADMIN_EMAIL)            { showModalError('suError', 'This email is reserved. Please use another.'); return; }
...
function signIn() {
  ...
  if (email === ADMIN_EMAIL && pass === ADMIN_PASSWORD) {
    CU = { id:'admin', name:ADMIN_NAME, email:ADMIN_EMAIL, isAdmin:true };
    CR = 'admin';
    modalHide('signinModal');
    saveSession(); updateNav();
    showToast('Welcome back, ' + ADMIN_NAME + '! Admin access granted ✓', 'ok');
...
/* ── AUTH: ADMIN LOGIN (credential-only, no bypass) ── */
// Admin must always use rakesh@gmail.com / rakesh@123 via the Sign In form.
// There is no demo/bypass button. signIn() handles admin detection automatically.
...
function signOut() {
  ...
  Store.del('session');
  // Hard-reset all upload UI so the next vendor starts clean
  resetVehicleForm();
...
function vendorSignUp() {
  ...
  if (email === ADMIN_EMAIL)          { showModalError('vuError', 'This email is reserved.'); return; }
  var vendors = Store.get('vendors') || [];
  if (vendors.find(function(v) { return v.email === email; })) {
    showModalError('vuError', 'Email already registered as a vendor.'); return;
  }
  var users = Store.get('users') || [];
  if (users.find(function(u) { return u.email === email; })) {
    showModalError('vuError', 'Email already registered as a user.'); return;
  }
  ...
  // Clear any previous vendor's upload state
  insDoc = null; upImgs = [];
  CU = v; CR = 'vendor';
...
function vendorSignIn() {
  ...
  // Clear any previous vendor's upload state before switching accounts
  insDoc = null; upImgs = [];
  CU = v; CR = 'vendor';
  modalHide('vendorSigninModal');
  saveSession(); updateNav();
  showToast('Welcome back, ' + v.name + '!', 'ok');
  if (v.status === 'pending') setTimeout(function() { showToast('Your account is pending admin approval', 'inf'); }, 600);
  showPage('vendor-dashboard');
}
...
function restoreSession() {
  ...
  // Refresh vendor status from store
  if (CR === 'vendor') {
  ...
  if (CR === 'user') {
    var freshUser = (Store.get('users') || []).find(function(u) { return u.id === CU.id; });
    if (freshUser) { CU = freshUser; saveSession(); }
  }
  updateNav();
}
...
function vehicleCardHtml(v, isTrending) {
  ...
  return '<div class="vcard" onclick="openDetail(\'' + v.id + '\')">' +
    '<div class="vcard-img">' +
      '<img src="' + img + '" alt="' + esc(v.name) + '" loading="lazy" onerror="this.src=\'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?w=600&h=400&fit=crop\'"/>' +
...
function renderPublicVehicles() {
  ...
  $('.ftag').each(function() {
    ...
    if (typeFilter === '' && onclick.indexOf("''") !== -1) $(this).addClass('active');
    else if (typeFilter && onclick.indexOf("'" + typeFilter + "'") !== -1) $(this).addClass('active');
  });
  $('#vehiclesGrid').html(
...
function updateBkSum() {
  ...
  if (d2 <= d1) {
    $('#bkSum').removeClass('show').html('');
    showToast('Return date must be after pickup date', 'err');
    return;
  }
...
function loadUserDash() {
  ...
  $('#pPhone').val(ud.phone || '');
  $('#pCity').val(ud.city || '');
  licFront = ud.licenseFront || null;
  licBack  = ud.licenseBack  || null;
  if (ud.licenseUploaded && (ud.licenseFront || ud.licenseBack)) {
    $('#licDone').show();
    if (ud.licenseFront) {
...
function handleLicUpload(input, side) {
  ...
  if (!file.type.match(/^image\//)) {
    showToast('Please upload an image file for your license', 'err');
    return;
  }
  if (file.size > 5 * 1024 * 1024) {
    showToast('File must be under 5 MB', 'err');
    return;
  }
  ...
  reader.onload = function(ev) {
    ...
    if (!dataUrl) { showToast('File read failed', 'err'); return; }
    ...
    if (side === 'front') {
      ...
      if (licFront && licBack) {
        // Save license data to current user
        if (CR === 'user' && CU) {
          var users = Store.get('users') || [];
          ...
          if (idx !== -1) {
            ...
            CU = users[idx];
            saveSession();
            showToast('License uploaded successfully! You can now book vehicles.', 'ok');
          }
        }
      } else {
        var missing = licFront ? 'back' : 'front';
        showToast('Great! Now upload the ' + missing + ' side to enable booking.', 'inf');
      }
  };
...
function deleteAccount() {
  if (!confirm('Are you sure you want to delete your account? All your bookings will be permanently removed. This action cannot be undone.')) return;
...
function submitContact() {
  ...
  if (!/\S+@\S+\.\S+/.test(e)) { showToast('Enter a valid email address', 'err'); return; }
  ...
  var rows = vs.map(function(v) {
    return '<tr>' +
      ...
      '<td><div class="acts">' +
        ...
        (v.status === 'rejected' ? '<button class="btn-de-success" onclick="setVendorStatus(\'' + v.id + '\',\'approved\')">Reinstate</button>' : '') +
        '<button class="btn-de-del" onclick="removeVendor(\'' + v.id + '\')"><i class="fas fa-trash"></i> Remove</button>' +
      '</div></td>' +
    '</tr>';
  }).join('');
  $('#aVendorsList').html(
...
function setVendorStatus(id, s) {
  ...
  renderVendorTbl(vs);
  showToast('Vendor ' + (s === 'approved' ? 'approved ✓' : s === 'rejected' ? 'suspended' : 'updated'), s === 'approved' ? 'ok' : 'inf');
}
function removeVendor(id) {
  if (!confirm('Remove this vendor? Their vehicles will also be permanently removed.')) return;
  ...
  var rows = vs.map(function(v) {
    ...
    return '<tr>' +
      ...
      '<td><div class="acts">' +
        ...
        (v.status === 'rejected' ? '<button class="btn-de-success" onclick="approveVehicle(\'' + v.id + '\')">Reinstate</button>' : '') +
...
function delVehicle(id) {
  if (!confirm('Permanently delete this vehicle? This action cannot be undone.')) return;
...
function renderUserTbl(us) {
  ...
  var rows = us.map(function(u) {
    var licCell = u.licenseUploaded
      ? '<span class="sbadge s-approved"><i class="fas fa-id-card"></i> Uploaded</span>' +
        (u.licenseFront ? ' <button class="btn-de-warn ms-1" style="font-size:.65rem;padding:3px 8px" onclick="viewLicense(\'' + u.id + '\')">View</button>' : '')
...
function removeUser(id) {
  if (!confirm('Remove this user and all their bookings? This cannot be undone.')) return;
  ...
  $('#aMessagesList').html(ms.map(function(m) {
    return '<div class="bcard">' +
      '<div class="bcard-hdr">' +
        ...
        '</div>' +
        '<div class="bcard-right">' +
          '<span style="font-size:.72rem;color:var(--text3)">' + new Date(m.sentAt).toLocaleDateString('en-IN') + '</span><br/>' +
...
function loadVendorDash() {
  if (!CU) return;
  // Always get fresh vendor data from store
  var freshVendor = (Store.get('vendors') || []).find(function(v) { return v.id === CU.id; });
  if (freshVendor) { CU = freshVendor; saveSession(); }
  ...
  $('#vendorPendingNotice').toggle(CU.status === 'pending');
  // Always reset the add vehicle form when loading vendor dashboard
  // so a different vendor never sees another vendor's uploads
  resetVehicleForm();
  loadVendorVehicles();
  loadVendorOrders();
}
...
function loadVendorOrders() {
  ...
  $('#vOrdersList').html(
    ords.length ? ords.map(function(b) { return vendorOrderCardHtml(b); }).join('') : emptyHtml('fa-bell-slash', 'No orders yet. Get your vehicles approved to start receiving bookings.')
  );
}

function vendorOrderCardHtml(b) {
  return '<div class="bcard">' +
    '<div class="bcard-hdr">' +
      '<div class="bcard-ico"><i class="fas fa-user"></i></div>' +
      '<div class="bcard-info">' +
        '<h4>' + b.vehicleName + '</h4>' +
        '<div class="bcard-dates"><i class="fas fa-user"></i> ' + b.userName + ' &nbsp;·&nbsp; <i class="fas fa-calendar"></i> ' + b.startDate + ' → ' + b.endDate + '</div>' +
      '</div>' +
      '<div class="bcard-right">' +
        '<div class="bcard-price">₹' + (b.total || 0).toLocaleString() + '</div>' +
        '<span class="sbadge s-approved">' + (b.status || 'confirmed') + '</span>' +
      '</div>' +
    '</div>' +
    '<div class="bcard-det">' +
      '<div class="bdet-grid">' +
        '<div class="bdet-item"><span class="bdet-lbl"><i class="fas fa-envelope" style="color:var(--accent)"></i> Customer Email</span><span class="bdet-val">' + (b.userEmail || '—') + '</span></div>' +
        '<div class="bdet-item"><span class="bdet-lbl"><i class="fas fa-phone" style="color:var(--accent)"></i> Customer Phone</span><span class="bdet-val">' + (b.userPhone || '—') + '</span></div>' +
        '<div class="bdet-item"><span class="bdet-lbl"><i class="fas fa-map-marker-alt" style="color:var(--accent)"></i> Pickup</span><span class="bdet-val">' + (b.pickupLocation || '—') + '</span></div>' +
        '<div class="bdet-item"><span class="bdet-lbl"><i class="fas fa-flag-checkered" style="color:var(--success)"></i> Return</span><span class="bdet-val">' + (b.returnLocation || '—') + '</span></div>' +
        '<div class="bdet-item"><span class="bdet-lbl"><i class="fas fa-calendar-alt" style="color:var(--accent)"></i> Duration</span><span class="bdet-val">' + b.days + ' day(s)</span></div>' +
        '<div class="bdet-item"><span class="bdet-lbl"><i class="fas fa-clock" style="color:var(--accent)"></i> Booked On</span><span class="bdet-val">' + (b.bookedAt ? new Date(b.bookedAt).toLocaleDateString('en-IN', {day:'2-digit',month:'short',year:'numeric'}) : '—') + '</span></div>' +
      '</div>' +
      '<div class="bdet-total-row">' +
...
function handleVehicleImages(input) {
  ...
  // Count only valid (non-null) existing images
  var validCount = upImgs.filter(function(i) { return i !== null && i !== undefined && i !== ''; }).length;

  if (validCount >= 5) {
    showToast('Maximum 5 images allowed. Remove one first.', 'err');
    return;
  }
  var remaining = 5 - validCount;
  var toProcess = files.slice(0, remaining);
  if (files.length > remaining) {
    showToast('Only ' + remaining + ' more image(s) allowed (max 5 total).', 'inf');
  }

  toProcess.forEach(function(f) {
    if (!f.type.startsWith('image/')) { showToast('Only image files are allowed', 'err'); return; }
    ...
    reader.onload = function(ev) {
      var data = ev.target.result;
      if (!data) return;
      // Always push to end — use array index as unique DOM id
      var idx = upImgs.length;
      upImgs.push(data);
      $('#imgPrevs').append(
        '<div class="img-pitem" id="ip_' + idx + '">' +
          '<img src="' + data + '" alt="Vehicle photo"/>' +
          '<button class="img-rm" onclick="removeImg(' + idx + ',event)" type="button" title="Remove"><i class="fas fa-times"></i></button>' +
        '</div>'
      );
      // Update counter label
      var newValidCount = upImgs.filter(function(i) { return i !== null && i !== undefined && i !== ''; }).length;
      $('#imgUpBox').html('<i class="fas fa-cloud-upload-alt"></i> ' + newValidCount + '/5 image(s) uploaded — click to add more');
    };
...
function removeImg(idx, e) {
  ...
  if (idx >= 0 && idx < upImgs.length) {
    upImgs[idx] = null; // null the slot — preserved in array so other indices stay stable
  }
  $('#ip_' + idx).remove();
  // Update counter label
  var validCount = upImgs.filter(function(i) { return i !== null && i !== undefined && i !== ''; }).length;
  if (validCount === 0) {
    ...
    $('#imgUpBox').html('<i class="fas fa-cloud-upload-alt"></i> ' + validCount + '/5 image(s) uploaded — click to add more');
  }
}
...
function addVehicle() {
  if (!CU || CR !== 'vendor') {
    showToast('Only vendors can add vehicles', 'err');
    return;
  }

  if (CU.status !== 'approved') {
    showToast('Your account is not approved yet', 'err');
    return;
  }
  ...
  if (!upImgs.length) {
    showToast('Upload at least one car image', 'err');
    return;
  }
  ...
  showToast('Vehicle submitted for approval', 'ok');

  resetVehicleForm(); // clear form
  loadVendorDash();
}
